...
#ifndef BLACKNET_CRYPTO_ABELIANGROUP_UNIFIED_H
#define BLACKNET_CRYPTO_ABELIANGROUP_UNIFIED_H
...
concept AbelianGroupElement = requires(T a, T b) {
    { T::additive_identity() } -> std::same_as<T>;
    { a + b } -> std::same_as<T>;
    { a - b } -> std::same_as<T>;
    { a.douple() } -> std::same_as<T>;
    { -a } -> std::same_as<T>;
};
...
concept FieldElement = requires(T a) {
    { T::additive_identity() } -> std::same_as<T>;
    { T::multiplicative_identity() } -> std::same_as<T>;
    { a.invert() } -> std::same_as<std::optional<T>>;
};
...
struct UnifiedConstraint {
    ...
    constexpr UnifiedConstraint() = default;
    constexpr UnifiedConstraint(Type t, VectorDense<AG>&& ins, AG out, bool cond = true)
        : type(t), inputs(std::move(ins)), output(out), condition(cond) {}
    
    // Convert from inputs vector for compatibility
    constexpr UnifiedConstraint(Type t, std::vector<AG>&& ins, AG out, bool cond = true)
...
struct UnifiedConstraintSystem {
    ...
    constexpr UnifiedConstraintSystem() = default;
    
    // Convert from legacy constraint vectors
    template<typename LegacyConstraint>
    constexpr UnifiedConstraintSystem(std::vector<LegacyConstraint>&& legacy)
...
AG multiply(const AG& e, const Scalar& s) noexcept {
    auto engine = create_addsubchain_engine<AG, Scalar>();
    auto result = engine->multiply_simple(e, s);
    return result ? *result : AG::additive_identity();
}

} // namespace blacknet::crypto::abeliangroup

#endif // BLACKNET_CRYPTO_ABELIANGROUP_UNIFIED_H